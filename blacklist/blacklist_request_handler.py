    ...
    def verify_challenge(self, pub_key, record_id, signed_nonce):
        ...
        if match is False:
            ...
            try:
                ...
                # make sure to convert public key from stored string value back to base 16 hex, or the hash will not match
                OP_HASH160 = subprocess.check_output('echo -n '+hex(int(pub_key, 16))+' | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160', shell=True)
                script_pub_key_string = OP_HASH160.decode('utf-8')
                ...
                result = self.datastore_client.write_permanent_blacklist_record([script_pub_key_string])
                return result
            except:
                logging.exception('')
